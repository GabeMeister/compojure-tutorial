(ns compojure-tutorial.components.slides.team-commits-curr-year
...
(defn team-commits-curr-year-slide
  [id-num]
  (let [repo-map (get-repo-by-id id-num)
        prev-slide-url-str (str "/presentation/" id-num "?" (get-prev-slide-query-params "team_commits" "curr_year_number"))
        next-slide-url-str (str "/presentation/" id-num "?" (get-next-slide-query-params "team_commits" "curr_year_number"))]
    (templ
     (page-wrapper [:div {:class "flex flex-col items-center p-6"}
                    [:div "This is the team commits CURR year slide for " (:repos/name repo-map)]
